...
test("resolveCommand runs vite from its JS entrypoint, never a bin shim", () => {
  const { command, args } = resolveCommand("vite", ["dev", "--port", "8080"]);
...
test("resolveCommand passes an explicit path through untouched", () => {
  const { command, args } = resolveCommand(process.execPath, ["-e", "0"]);
  assert.equal(command, process.execPath);
