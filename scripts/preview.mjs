...
// npm is a `.cmd` shim on Windows — `spawn("npm")` cannot exec the bare name.
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (process.platform === "win32") {
    console.error(
      '[preview] preview:stop/preview:restart are Linux-sandbox-only — run "npm run preview" ' +
        "(vite preview) directly on Windows",
    );
    process.exit(1);
  }
