...
export function isMainModule(moduleUrl) {
  ...
  try {
    ...
    if (process.platform === "win32") {
      // Windows paths compare case-insensitively but string equality does not:
      // a `c:` vs `C:` drive-letter difference would turn the wrapper into a
      // silent no-op that exits 0 without starting Vite.
      return realpathSync(entry).toLowerCase() === target.toLowerCase();
    }
...
/**
 * Resolve a command to a `{ command, args }` spawn tuple that never depends on
 * an npm bin shim. npm's install writes `<bin>` (POSIX sh) and `<bin>.cmd`
 * (Windows); the sh is meaningless to a Windows node, and `spawn()` of the
 * `.cmd` shim goes through `cmd.exe`, which WSL interop refuses (EINVAL).
 *
 * `vite` — the only command the package.json scripts ever start — is therefore
 * resolved straight to its JS entrypoint and run with node, which is exactly
 * what the shim does (`node …/vite/bin/vite.js`). Any other bare bin name
 * falls back to the `.cmd` suffix on Windows; an explicit path is left alone.
 */
export function resolveCommand(command, args) {
  if (command === "vite") {
    const viteBin = join(projectRoot(), "node_modules", "vite", "bin", "vite.js");
    return { command: process.execPath, args: [viteBin, ...args] };
  }
  if (process.platform === "win32" && basename(command) === command && !extname(command)) {
...
function main(argv) {
  ...
  const { command: cmd, args: cmdArgs } = resolveCommand(command, args);
  const child = spawn(cmd, cmdArgs, { stdio: "inherit", env, windowsHide: true });
